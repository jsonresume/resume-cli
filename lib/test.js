...
// makes the schemas output human readable from the console
function errorFormatter(errors, readFileErrors) {
    //check if there are schcema errors
    if (readFileErrors) {
...
function validate(resumeData, readFileErrors, callback) {
    console.log('\n  running validation tests on resume.json ... \n');
    resumeSchema.validate(resumeData, function(report, errs) {
        if (errs || readFileErrors) {
            callback(true);
            errorFormatter(errs, readFileErrors);
