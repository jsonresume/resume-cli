...
module.exports = function exportResume(resumeJson, fileName, theme, format, callback) {
  if (!fileName) {
    ...
    if (formatToUse === '.html') {
      createHtml(resumeJson, fileName, theme, formatToUse, function (error) {
        if (error) {
      ...
      createPdf(resumeJson, fileName, theme, formatToUse, function (error) {
        if (error) {
...
function createHtml(resumeJson, fileName, theme, format, callback) {
  ...
  const stream = fs.createWriteStream(path.resolve(process.cwd(), fileName + format));
