...
function publishMenu(program, themeChanger, callback) {
    if (themeChanger.changeTheme === true) {
        callback(customPublish, themeChanger.theme, program);
    } else {
        ...
        menu.write('Or simply publish as guest.'.white);
        menu.write('Or simply publish as guest.'.white);
        menu.on('select', function(label) {
            menu.close();
            console.log('\nSELECTED: ' + label);
            callback(label, program.theme);
        });
...
function publish(resumeJson, program, themeChanger) {

    // if (resumeJson === null) {
    //     console.log(program.theme);
    //     console.log(program.changeTheme);
    //     // return;
    // }
    ...
    publishMenu(program, themeChanger, function(label, theme, program) {
        ...
        if (label === guestPublish) {
        ...
        } else if (label === customPublish || themeChanger.changeTheme === true) {

            console.log(themeChanger.theme);
            console.log(themeChanger.changeTheme);
            console.log(themeChanger.email);
            console.log(themeChanger.password);
            // return;

            if (themeChanger.changeTheme) {
                publishSend(null, themeChanger.theme, themeChanger.email, themeChanger.password, false, false);
                return;
            }
            ...
            read({
                prompt: "email: ",
                default: reumeJson && resumeJson.bio && resumeJson.bio.email && resumeJson.bio.email.personal || '' // need first one?
            }, function(er, email) {
                if (er) {
                    console.log('');
                    process.exit();
