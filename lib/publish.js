...
//publish menu
var customPublish = 'Publish with a customized domain name.';
var guestPublish = 'Publish with a generated domain name.';
var encriptedGuest = 'Publish an encrypted resume with a generated domain name.';
var encriptedCustom = 'Publish an encrypted resume with a customized domain name.';
...
function publishMenu(callback) {
    ...
    menu.add('EXIT');
    menu.write('\n');
    menu.write('To acquire login details for a customized domain name, Press ^C and type: `resume register`\n'.white);
...
function publish(resumeJson, force) {
    publishMenu(function(label) {
        if (label === guestPublish) {
            ...
            console.log('If you already have an account, enter your login details below:'.cyan);
            read({
                prompt: "email: ",
                    ...
                    read({
                        ...
                        resumeSchema.validate(resumeJson, function(report, errs) {
                            if (errs && !force) {
                                ...
                                spinner.start();
                                publishSend(resumeJson, email, password, false, passphrase);
                            }
                        });
                    });
                });
            ...
            resumeSchema.validate(resumeJson, function(report, errs) {
                if (errs && !force) {
                    ...
                    read({
                        ...
                        spinner.start();
                        publishSend(resumeJson, null, null, true, passphrase);
                    });
                }
            });
...
function publishSend(resumeJson, email, password, guest, passphrase) {
    request
