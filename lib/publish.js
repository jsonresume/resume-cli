...
function publishMenu(callback) {
    ...
    menu.add('EXIT');
    menu.write('\n');
    menu.write('Type ' + 'resume register '.white + 'to acquire login details for custom domain names.\n'.cyan);
        ...
        if (label === guestPublish) {
            ...
            resumeSchema.validate(resumeData, function(report, errs) {
                if (errs && !force) {
                    ...
                    spinner.start();
                    publishSend(resumeData, null, null, guest, false);
                }
            });
                ...
                read({
                    ...
                    resumeSchema.validate(resumeData, function(report, errs) {
                        if (errs && !force) {
                            ...
                            spinner.start();
                            publishSend(resumeData, email, password, false, false);
                        }
                    });
                });
            });
                    ...
                    read({
                        ...
                        resumeSchema.validate(resumeData, function(report, errs) {
                            if (errs && !force) {
                                ...
                                spinner.start();
                                publishSend(resumeData, email, password, false, pProtected);
                            }
                        });
                    });
                });
            ...
            resumeSchema.validate(resumeData, function(report, errs) {
                if (errs && !force) {
                    ...
                    read({
                        ...
                        spinner.start();
                        publishSend(resumeData, null, null, true, pProtected);
                    });
                }
            });
...
function publishSend(resumeData, email, password, guest, pProtected) {
    request
