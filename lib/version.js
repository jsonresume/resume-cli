...
function writeToConfig(authObj) {
    var jsonConfigFile = homeDir + '/.jsonresume.json';
    jsonConfigFileParamIndex = process.argv.indexOf('-c');
    if(process.argv.length > jsonConfigFileParamIndex){
        jsonConfigFile = process.argv[jsonConfigFileParamIndex + 1];
    }
    fs.writeFileSync(jsonConfigFile, JSON.stringify(authObj, undefined, 2));
}
