...
function login(config, resumeJson) {
    read({
        prompt: "email: "
    }, function(er, email) {
        if (er) {
...
function logMeIn(config, userInfo) {
    spinner.start();
    request
...
function writeConfig(config, authObj, callback) {
    if(config == null) { config = homeDir + '/.jsonresume.json'; }
    fs.readFile(config, 'utf8', function(fileDoesNotExist, config) {
        // console.log(authObj);
        ...
        // console.log(config);
        fs.writeFileSync(config, JSON.stringify(config, undefined, 2));
        //todo: use async
        callback(true);
    });
}
