    ...
    if (!fileName) {
        ...
        switch (fileExtension) {
            case 'html':
                ...
                break;
            case 'md':
                resumeToMarkdown(resumeJson, function(markdownResume) {
                    fs.writeFileSync(fileName, markdownResume, 'utf8');
                    console.log('Done! Find your generated .md resume at:'.green, process.cwd() + '/' + fileName);
                    console.log('To publish your resume at:'.cyan, 'http://jsonresume.org', 'Type the command:'.cyan, 'resume publish');
                    callback(true, fileName);
                });
                break;
                    ...
                    .end(function(error, res) {
                        ...
                        if (error && error.code === 'ENOTFOUND') {
                            ...
                            download(res.body.url + '.pdf', fileName, function(err, data) {
                                ...
                                // only returns undefined undefined
                                // do nothing?
                            })
